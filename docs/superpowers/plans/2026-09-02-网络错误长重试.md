...
```java
        // 每轮：TOOL 一次 + NETWORK 若干次（长重试直至墙钟耗尽，次数随 sleep 精度浮动）
        assertTrue("每轮至少 2 次网络请求: " + net.calls, net.calls >= 6);
        assertTrue("不得无限重试: " + net.calls, net.calls <= 20);
```
...
```java
                    ...
                    } else if (degradeImagesOnFailure() && noOutputYet(content, thinking)) {
...
```java
                if ("error".equals(finish[0])) {
                    if (degradeImagesOnFailure() && noOutputYet(content, thinking)) continue; // 同上：onError 回调路径（如 API 400）
