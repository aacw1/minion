...
**Interfaces:**
- Produces: `AgentUi.onRetryProgress(int attempt)`（default 空实现）；`RunningIndicator.setRetryProgress(int attempt)`（≥1 显示"正在重试中…第 N 次"停轮换，0 恢复）；静态 `RunningIndicator.retryText(int)`；`RecordingUi.retryProgress`（List&lt;Integer&gt;）
                        ...
                        while (true) {
                            ...
                            if (retryPolicy429.isExhausted(waited)) {
                                ui.onError("子 agent 429 重试了 " + attempts + " 次，持续 "
                                        + (waited / 60000) + " 分钟仍失败，已停止重试");
                                return "子 agent 失败: 429 持续 " + (waited / 60000) + " 分钟";
                            }
                            ui.onRetryProgress(attempts); // 指示器显示"正在重试中…第 N 次"
                            try {
...
- 对照设计文档逐条核对：参数（2000/2000/10000/1800000）、重试进度（onRetryProgress 每次重试更新 + 退出复位 0，指示器"正在重试中…第 N 次"）、消息区仅成功"已恢复"/超时总结各一条、中断响应（≤100ms）、500/网络/超时仍 1 次、无新增配置项
