...
```java
    /** 429 限流长重试：首失败提示"重试中"→ 静默 → 成功后"已恢复"，无刷屏无报错 */
    @Test
    public void rateLimit_retryThenSuccess_silentRecovery() {
        ...
        assertEquals(2, ui.warnings.size());  // "重试中" + "已恢复"，无其他刷屏
        assertTrue(ui.warnings.get(0).contains("正在重试中"));
        assertTrue(ui.warnings.get(1).contains("已恢复"));
...
```java
                    ...
                    if (e.type == LlmException.Type.RATE_LIMIT) {
                        ...
                        // 首次提示后静默，成功轻提示恢复，超时一次性总结停止
                        int attempts = 0;
                        long waited = 0;
                        ui.onWarning("请求失败(429)，正在重试中…");
                        while (true) {
                            attempts++;
...
```java
    /** 子 agent 429 长重试：先 429 后成功，提示一次"重试中"+"已恢复"，与主循环一致 */
    @Test
    public void subAgent_rateLimit_retryThenSuccess() throws Exception {
        ...
        assertEquals(2, llm.requests.size()); // 原始请求 + 1 次重试
        assertEquals(2, ui.warnings.size());
        assertTrue(ui.warnings.get(0).contains("正在重试中"));
        assertTrue(ui.warnings.get(1).contains("已恢复"));
        assertTrue(ui.errors.isEmpty());
    }
...
```java
                    ...
                    if (e.type == LlmException.Type.RATE_LIMIT) {
                        ...
                        int attempts = 0;
                        long waited = 0;
                        ui.onWarning("子 agent 请求失败(429)，正在重试中…");
                        while (true) {
                            attempts++;
                            long delay = retryPolicy429.delayMs(attempts);
...
- 对照设计文档逐条核对：参数（2000/2000/10000/1800000）、静默提示（首错 1 次"重试中"+ 成功"已恢复"+ 超时总结）、中断响应（≤100ms）、500/网络/超时仍 1 次、无新增配置项
- `git log --oneline -6` 确认 5 个提交完整
