...
**Interfaces:**
- Produces: `AgentUi.onRetryProgress(int attempt)`（default 空实现）；`RunningIndicator.setRetryProgress(int attempt)`（≥1 显示"429限流或余额不足，正在重试中...N次"停轮换，0 恢复）；静态 `RunningIndicator.retryText(int)`；`RecordingUi.retryProgress`（List&lt;Integer&gt;）
...
- 对照设计文档逐条核对：参数（2000/2000/10000/1800000）、重试进度（onRetryProgress 每次重试更新 + 退出复位 0，指示器"429限流或余额不足，正在重试中...N次"）、消息区无重试警告（静默恢复）、仅超时总结一条、中断响应（≤100ms）、500/网络/超时仍 1 次、无新增配置项
